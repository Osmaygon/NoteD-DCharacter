...
export function CharacterSheetPage({ demoCharacterId }: { demoCharacterId?: string }) {
  const params = useParams<{ id: string }>();
  const characterId = demoCharacterId ?? params.id;
  const isDemo = Boolean(demoCharacterId);
  ...
  const loadData = useCallback(async (uid: string, characterId: string) => {
    const detail = isDemo
      ? ((await (await fetch("/api/demo/talhur")).json()) as { character?: CharacterDetail }).character ?? null
      : await getCharacterDetail(uid, characterId);
    if (!detail) {
  ...
  const loadLevelSnapshots = useCallback(async () => {
    ...
    if (!token) return;
    const response = await fetch(`/api/characters/${characterId}/levels?appSessionToken=${encodeURIComponent(token)}`);
  ...
  async function activateLevel(level: number) {
    ...
    try {
      ...
      await loadData(userId, characterId);
  ...
  useEffect(() => {
    void (async () => {
      ...
      setUserId(user?.user_id ?? "demo");
      await loadData(user?.user_id ?? "demo", characterId);
      await loadLevelSnapshots();
      setActiveStatuses(isDemo || !user ? [] : await listActiveStatusEffects(user.user_id, characterId));
    })();
  }, [loadData, loadLevelSnapshots, characterId, isDemo]);
  ...
  async function onSave() {
    if (!userId) return;
    if (isDemo) {
      setMessage("Modo demo: los cambios no se guardan.");
      return;
    }
    try {
      setMessage("");
      await updateCharacterDetail(userId, characterId, {
        name: form.name,
  ...
  async function applyRest(kind: RestKind) {
    ...
    try {
      ...
      if (resetSpellSlots) {
        if (!isDemo) await updateCharacterSpellSlots(userId, characterId, {});
  ...
  async function onDelete() {
    if (!userId) return;
    if (isDemo) {
      setMessage("Modo demo: no se puede borrar el personaje real.");
      return;
    }
  ...
  async function persistInventory(next: Partial<InventoryState>) {
    ...
    if (isDemo) return;
    await updateCharacterInventory(userId, characterId, normalizedNext);
  }
  ...
  async function saveManualTrait(trait: TraitEntry) {
    ...
    if (!isDemo) await updateCharacterSourcePayload(userId, characterId, nextPayload);
  ...
  async function toggleSpentSpellSlot(level: string, index: number) {
    ...
    if (!isDemo) await updateCharacterSpellSlots(userId, characterId, next);
  ...
  async function refreshActiveStatuses() {
    if (!userId) return;
    if (!isDemo) setActiveStatuses(await listActiveStatusEffects(userId, characterId));
  }

  async function toggleStatus(status: StatusEffect, active: boolean) {
    if (!userId) return;
    if (isDemo) {
      setActiveStatuses((current) => active ? [...current, status] : current.filter((entry) => entry.id !== status.id));
    } else {
      await setCharacterStatusEffectActive(userId, characterId, status.id, active);
  ...
  async function persistAmmunition(next: AmmunitionState) {
    if (!userId) return;
    if (!isDemo) await updateCharacterAmmunition(userId, characterId, next);
